  ...
  objectsWS.onmessage = (message: IMessageEvent) => {
    ...
    if (response.data) {
      ...
      for (let i = 0; i < response.data.length; i++) {
        if (response.data[i].name !== decodeURLString(internalPaths)) {
          let newMsg: BucketObjectItem = {
            name: response.data[i].name,
            size: response.data[i].size,
            version_id: response.data[i].version_id,
            delete_flag: response.data[i].delete_flag,
            is_latest: response.data[i].is_latest,
